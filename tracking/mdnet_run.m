function [ time, result ] = mdnet_run(images, region, net, display)
% MDNET_RUN
% Main interface for MDNet tracker
%
% INPUT:
%   images  - 1xN cell of the paths to image sequences
%   region  - 1x4 vector of the initial bo177,51,21,129unding box [left,top,width,height]
...
%simplenn_caffe_deploy(net_conv, 'conv_test');
%simplenn_caffe_deploy(net_fc, 'fc_test');
...
if(opts.bbreg)
    ...
    %bbox_reg = train_bbox_regressor_original(X, bbox, bbox_gt);
    bbox_reg = train_bbox_reg_shell(X, bbox, bbox_gt);
end
...
for To = 2:nFrames;
    ...
    if(opts.bbreg && target_score>0)
        ...
        %pred_boxes = predict_bbox_regressor_original(bbox_reg.model,
        %X_,bbox_);
        pred_boxes = predict_bbox_regressor_shell(bbox_reg.model, X_, bbox_);
